...
uint32_t Networking_SetCalibration(void)
{
	...
	{
		A7128_StrobeCmd(CMD_STBY);
		A7128_TxFIFO[pTargetIDIndex]=PCM_RxBuf[pCP1616_Master_DataIndex];
		A7128_TxFIFO[pSourceIDIndex]=MasterAddr;		
		A7128_TxFIFO[pCommandIndex]=SetCalibrationCommand;
		A7128_TxFIFO[pLenIndex]=12;	
		for(i=0;i<12;i++)
			A7128_TxFIFO[pDataIndex+i]=PCM_RxBuf[pCP1616_Master_DataIndex+1+i];	
		A7128_WriteFIFO();
		A7128_StrobeCmd(CMD_TX); 
		while(A7128_GIO1_Read()==1);//发送完成
		A7128_StrobeCmd(CMD_RFR);
		...
		{	
			if(A7128_GIO1_Read()==0)//收到数据，判断数据的正确性
			{
				...
				if((x&0x20)==0)//判断是否CRC校验出错
				{
					...
					{
						if(A7128_RxFIFO[pSourceIDIndex]==PCM_RxBuf[pCP1616_Master_DataIndex])
						{
							return 1;
						}
					}					 	
...
{	
	...
	if(PCM_RxBuf[pCP1616_Master_CommandIndex]==1)//启动网络
	{
		...
		{
			PCM_TxBuf[pCP1616_Master_DataIndex]=2;//网络已经启动，自动接入即可
		}
		else if(NetWorkMode ==Net_Start_Calibration)
		{
			PCM_TxBuf[pCP1616_Master_DataIndex]=3;//标定网络已经启动，将自动转为测量网络，自动接入即可
		}
...
// 			bRxConfirm=1;
	...
	if(PCM_RxBuf[pCP1616_Master_CommandIndex]==5)//启动标定网络
	{
		bCalibrationNet=1;
		if(NetWorkMode==Net_Stop)
		{			
		...
		{
			PCM_TxBuf[pCP1616_Master_DataIndex]=3;//测量网络已经启动，将转为标定网络，自动接入
		}
		HAL_UART_Transmit(&huart1,PCM_TxBuf,8,1000);
		HAL_UART_Receive_DMA(&huart1,PCM_RxBuf,PCM_RxFrameLen);	
	}
	else if(PCM_RxBuf[pCP1616_Master_CommandIndex]==6)//设定标定系数
	{
		if(NetWorkMode==Net_Stop)
		{
	...
	else if(PCM_RxBuf[pCP1616_Master_CommandIndex]==7)//读取标定系数
	{
		if(NetWorkMode==Net_Stop)
		{
			if(Networking_GetCalibration()==1)
			{
				PCM_TxBuf[4]=1+13;				
				PCM_TxBuf[pCP1616_Master_DataIndex]=1;//成功设定
				PCM_TxBuf[pCP1616_Master_DataIndex+1]=A7128_RxFIFO[pSourceIDIndex];
				for(i=0;i<12;i++)
					PCM_TxBuf[pCP1616_Master_DataIndex+2+i]=A7128_RxFIFO[pDataIndex+i];
				PCM_TxBuf[20]=0x0d;
				HAL_UART_Transmit(&huart1,PCM_TxBuf,8+13,1000);
				HAL_UART_Receive_DMA(&huart1,PCM_RxBuf,PCM_RxFrameLen);	
			}
			else
			{
...
void Uart_ClosedProc(void)
{
	...
	{		
		...
		{	
			...
			{
				UartRxProc();
				if(PCM_RxBuf[pCP1616_Master_CommandIndex]==2)
				{
					if(PCM_RxBuf[pCP1616_Master_DataIndex]==0x02)
					{
						return;				
					}
				}	
