...
{
  uint8 ip[4];
  w5500_SetMac(ConfigMsg.mac);/*配置Mac地址*/
  w5500_SetSubnetMask(ConfigMsg.sub);/*配置子网掩码*/
  w5500_SetGateWay(ConfigMsg.gw);/*配置默认网关*/
  w5500_SetIP(ConfigMsg.lip);/*配置Ip地址*/

  //Init. TX & RX Memory size of w5500
  sysinit(txsize, rxsize); /*初始化8个socket*/
  
  w5500_Set_RetransmissionTime(2000);/*设置溢出时间值*/
  w5500_Set_NumOfRetransmission(3);/*设置最大重新发送次数*/
