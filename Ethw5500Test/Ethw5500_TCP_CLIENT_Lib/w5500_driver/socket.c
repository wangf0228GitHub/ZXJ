...
uint8 socket(SOCKET s, uint8 protocol, uint16 port, uint8 flag)
{
      ...
      )
   {
      ...
      if (port != 0) {
         ...
         w5500_WriteByte(Sn_PORT0(s) ,(uint8)((local_port & 0xff00) >> 8));
         w5500_WriteByte(Sn_PORT1(s) ,(uint8)(local_port & 0x00ff));
      }
...
uint8 connect(SOCKET s, uint8 * addr, uint16 port)
{
    ...
    else
    {
        ...
        while ( w5500_READ(Sn_SR(s)) != SOCK_SYNSENT )
        {
            ...
            if (w5500_Get_SocketN_InterruptStatus(s) & Sn_IR_TIMEOUT)
            {
...
uint16 send(SOCKET s, const uint8 * buf, uint16 len)
{
  ...
  do
  {
    freesize = w5500_Get_SocketN_FreeTxBufSize(s);
  ...
  // copy data
  w5500_SendDataProc(s, (uint8 *)buf, ret);
...
uint16 recvfrom(SOCKET s, uint8 * buf, uint16 len, uint8 * addr, uint16 *port)
{
   ...
   if ( len > 0 )
   {
      ...
      {
      case Sn_MR_UDP :
        w5500_ReadBuf(addrbsb, head, 0x08);        
        ptr += 8;
        ...
        *port    = (*port << 8) + head[5];
        ...
        addrbsb = (uint32)(ptr<<8) +  (s<<5) + 0x18;
        w5500_ReadBuf(addrbsb, buf, data_len);                
        ptr += data_len;
      ...
      case Sn_MR_IPRAW :
        w5500_ReadBuf(addrbsb, head, 0x06);        
        ptr += 6;
        addr[0]  = head[0];
        addr[1]  = head[1];
        addr[2]  = head[2];
      ...
      case Sn_MR_MACRAW :
        w5500_ReadBuf(addrbsb, head, 0x02);
        ptr+=2;
        data_len = head[0];
...
{
  w5500_WriteByte(Sn_CR(s),Sn_CR_SEND_KEEP);
  return;
}
...
uint16 macraw_send( const uint8 * buf, uint16 len )
{
   ...
   w5500_SendDataProc(sock_num, (uint8 *)buf, len);

   //W5500 SEND COMMAND
   w5500_WriteByte(Sn_CR(sock_num),Sn_CR_SEND);
   while( w5500_READ(Sn_CR(sock_num)) );
   while ( (w5500_READ(Sn_IR(sock_num)) & Sn_IR_SEND_OK) != Sn_IR_SEND_OK );
...
uint16 macraw_recv( uint8 * buf, uint16 len )
{
   ...
   if ( len > 0 )
   {
      ...
      ptr = w5500_READ(Sn_RX_RD0(sock_num));
      ptr = (uint16)((ptr & 0x00ff) << 8) + w5500_READ( Sn_RX_RD1(sock_num) );
